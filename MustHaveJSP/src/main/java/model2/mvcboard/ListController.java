...
public class ListController extends HttpServlet{	// HttpServlet 을 상속
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) 
		...
		// 모델로 전달할 검색 매개변수 및 뷰로 전달할 페이징 관련 값을 저장하기 위해 Map 컬렉션을 생성
		...
		if(searchWord != null) { 
			// 쿼리스트링으로 전달 받은 매개변수 중 검색어가 있다면 map에 저장
			map.put("searchField", searchField);
			map.put("searchWord", searchWord);
		}
		int totalCount = dao.selectCount(map);	// 개시물 개수
